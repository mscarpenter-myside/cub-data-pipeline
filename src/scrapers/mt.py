...
class ScraperMT(BaseScraper):
    """
    Scraper for Mato Grosso (MT) CUB data.
    ...
    Same logic as PE/DF scrapers with MT-specific dropdown values.
    """
    ...
    def __init__(self, headless: bool = True):
        """Initialize the MT scraper with parent attributes."""
        super().__init__(
            estado="MT",
            base_url="http://cub.org.br/cub-m2-estadual/MT/"
        )
    ...
    def check_availability(self, month: int, year: int) -> bool:
        ...
        logger.info(f"Checking availability for MT - {month_name}/{year}")
    ...
    def extract(self, month: int, year: int) -> CUBData:
        ...
        2. Navigate through cascade of dropdowns (MT specific)
        3. Click "Gerar Relatório em PDF"
        4. Download and parse PDF
        """
        month_name = month_name_pt(month)
        logger.info(f"Extracting CUB data for MT - {month_name}/{year}")
        
        # Setup paths
        raw_path = get_data_path("raw")
        pdf_filename = f"cub_mt_{year}_{month:02d}.pdf"
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                logger.info("Filling form cascade for MT...")
                
                # Estado: MT - Mato Grosso (usually pre-selected from URL)
                ...
                for selector in sinduscon_selectors:
                    try:
                        ...
                        if dropdown.count() > 0:
                            # Try label first, then index fallback
                            try:
                                page.select_option(selector, label="Sinduscon-MT")
                            except:
                                page.select_option(selector, index=1)
                            page.wait_for_timeout(500)
                            logger.debug("Selected Sinduscon-MT")
                            break
                    except:
                        continue
...
if __name__ == "__main__":
    scraper = ScraperMT(headless=False)
    
    month, year = 12, 2025
    
    print(f"\n{'='*50}")
    print(f"Testing MT Scraper for {month}/{year}")
    print(f"{'='*50}\n")
