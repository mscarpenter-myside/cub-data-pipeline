...
class ScraperPE(BaseScraper):
    """
    Scraper for Pernambuco (PE) CUB data.
    ...
    Navigation:
    - Cascade of dropdowns: Estado -> Sinduscon -> Relatório -> Ano -> Mês -> CUB -> Variação
    ...
    def __init__(self, headless: bool = True):
        """Initialize the PE scraper with parent attributes."""
        super().__init__(
            estado="PE",
            base_url="http://cub.org.br/cub-m2-estadual/PE/"
        )
    ...
    def check_availability(self, month: int, year: int) -> bool:
        ...
        logger.info(f"Checking availability for PE - {month_name}/{year}")
    ...
    def extract(self, month: int, year: int) -> CUBData:
        ...
        2. Navigate through cascade of dropdowns
        ...
        """
        month_name = month_name_pt(month)  # Capitalized
        logger.info(f"Extracting CUB data for PE - {month_name}/{year}")
        
        # Setup paths
        raw_path = get_data_path("raw")
        pdf_filename = f"cub_pe_{year}_{month:02d}.pdf"
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                # Step B: Form Interaction (The Cascade)
                logger.info("Filling form cascade...")
                
                # Estado: PE - Pernambuco (usually pre-selected from URL)
                ...
                for selector in sinduscon_selectors:
                    try:
                        ...
                        if dropdown.count() > 0:
                            page.select_option(selector, label="Sinduscon-PE")
                            page.wait_for_timeout(500)
                            logger.debug("Selected Sinduscon-PE")
                            break
                    except:
                        continue
                ...
                # CUB/m²: Sem desoneração da mão de obra
                cub_selectors = [
                    "select#cub",
                    "select[name*='cub' i]",
                    "select#desoneracao",
                        ...
                        if dropdown.count() > 0:
                            ...
                            for label in labels_to_try:
                                try:
                                    page.select_option(selector, label=label)
                                    page.wait_for_timeout(500)
                                    logger.debug(f"Selected CUB type: {label}")
                                    break
                                except:
                                    continue
                            break
                ...
                # Step C: Download
                # Find and click the "Gerar Relatório em PDF" button
    ...
    def _parse_pdf(self, pdf_path: Path) -> float:
        ...
        with pdfplumber.open(str(pdf_path)) as pdf:
            ...
            # Final sanity check
            if x0 >= x1:
                logger.warning(f"Crop bounds invalid. Resetting to wide column.")
            ...
            for line in text.split('\n'):
                if re.search(r'\bR[-\s]?8\b', line, re.IGNORECASE):
                    ...
                    if match:
                        ...
                        # Try any currency value on the line
    ...
    def _find_r8_fallback(self, page) -> float:
        ...
        for line in text.split('\n'):
            if re.search(r'\bR[-\s]?8\b', line, re.IGNORECASE):
                ...
                # Value directly after R-8
                match = re.search(r'R-?8.*?(\d{1,3}(?:\.\d{3})*,\d{2})', line)
                if match:
                    val_str = match.group(1)
                    return self._parse_brl_currency(val_str)
                
                # Position-based (index 1 = NORMAL column)
...
if __name__ == "__main__":
    scraper = ScraperPE(headless=False)
    
    month, year = 12, 2025
    
    print(f"\n{'='*50}")
    print(f"Testing PE Scraper for {month}/{year}")
    print(f"{'='*50}\n")
